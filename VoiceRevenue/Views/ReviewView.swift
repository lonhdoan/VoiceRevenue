        ...
        NavigationView {
            VStack {
                List {
                    ForEach($model.candidates) { $item in
                        Section {
                            ...
                            TextField(
                                ...
                                text: Binding(
                                    get: {
                                        item.amountVND.map(String.init) ?? ""
                                    },
                                    set: {
                                        let digits = $0.filter(\.isNumber)
                                        item.amountVND = digits.isEmpty
                                            ? nil
                                            : Int64(digits)
                                    }
                                )
                    ...
                    .onDelete {
                        model.candidates.remove(atOffsets: $0)
                    }
...
private extension Binding where Value == String {
    init(
        ...
        replacingNilWith fallback: String
    ) {
        self.init(
            get: {
            ...
            set: { newValue in
                source.wrappedValue = newValue.isEmpty
                    ? nil
                    : newValue
            }
        )
...
private extension Binding where Value == Date {
    init(
        _ source: Binding<Date?>,
        replacingNilWith fallback: Date
    ) {
        self.init(
            get: {
                source.wrappedValue ?? fallback
